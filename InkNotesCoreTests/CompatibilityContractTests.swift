...
struct CompatibilityContractTests {
  ...
  func browserExcerptFlowRemainsUserInitiated() throws {
    ...
    #expect(browserSource.contains("showingSavedSources = true"))
    #expect(browserSource.contains("Label(\"打开网页\", systemImage: \"safari\")"))
    #expect(browserSource.contains("Label(\"删除来源\", systemImage: \"trash\")"))
    #expect(browserSource.contains("title: Text(\"删除这个来源？\")"))
    #expect(browserSource.contains("try await store.deletePageSource(source.id, from: pageID)"))
  ...
  func goldenVersionOneArchiveRemainsCompatible() async throws {
    ...
    #expect(restoreResult.repairedSourceCount == 0)
    #expect(restoreResult.repairedSourcePageIDs.isEmpty)
    #expect(restoreResult.selectedPageSources.isEmpty)
