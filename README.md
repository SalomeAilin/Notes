...
- 系统保存位置返回成功后，应用会通过安全文件协调重新读取刚保存的文件并与本次不可变备份逐字节比较；只有完全一致才记录保存时间和当时的笔记本、页数，并据此提示保存后是否有新修改。读取失败、取消或内容不一致都不更新状态；旧的仅保存回调记录按无法确认处理。应用不记录笔记内容、名称、文件路径或网盘账号，也不把回读一致解释成文件会长期存在或网盘已经同步。网盘结果以用户实际看到的远端文件为准。
