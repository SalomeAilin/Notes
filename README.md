...
- 已严格匹配路径、大小、MD5 和备份标识的常规 `create` 结果只返回“响应元数据匹配、内容未证明”，并和秒传、响应未知一样保留在待远端证明集合；同一会话也不会把它升级为已完成。这里的持久化记录是保守的 at-most-once 尝试屏障，只有可信 broker 强回执或安全下载后核对完整 `archiveSHA256` 才能证明远端内容。
...
- UInfo 账户身份探针仍不负责签发账号作用域，也未接入 App、View 或 Store。当前核心新增了隔离的同账号只读观察器：它只按固定父目录、最多 10 页/1,000 项调用官方 `list`，每页请求前都会重检凭据剩余有效期，以 UTF-8 字节精确匹配绝对路径，并把重复条目、`fs_id` 漂移、分页上限、畸形响应和账号错配全部失败关闭。普通 offset 分页不是服务端一致快照，因此多页未命中只能返回“不确定”；单个短页未命中也明确标为“缺席未证明”。即使路径、大小和云端哈希都匹配，也只返回“元数据一致但内容未证明”，不会删除 WAL、授权重传或标记成功；官方列表的云端哈希不能替代本地归档字节的 SHA-256/MD5 证明。仓库仍没有真实 broker、生产凭据发行器、自动刷新、自动核销或“强制清除”入口。
- 当前核心另有未接 UI 的完整字节核对器：它按同账号记录与 `fs_id` 查询官方 `filemetas`，并在元数据请求和后续下载请求前分别重检凭据剩余有效期；只把短期 access token 发送给固定的 `pan.baidu.com` 元数据端点和首跳 `d.pcs.baidu.com` 下载主机。后续只允许不再携带 token 的 HTTPS 重定向，并移除认证、Cookie、Referer、Origin 与 Range。下载使用无持久缓存的临时会话，拒绝压缩和分段响应，在 32 MiB 上限内流式计算完整 SHA-256；只有账号、备份、尝试、路径、官方 `filename`、大小、字节数和 SHA-256 全部一致才返回内存中的 `contentVerified`。该核对器没有 repository、lease、`FileManager` 或删除能力，不能核销 WAL；当前应用产物也没有生产凭据发行器或调用入口，尚未用真实百度账号验证 dlink/CDN 行为。这个本地门禁只能拒绝明显不可用的凭据，不能刷新 token，也不能保证长请求在到期前完成。
...
- 百度上传记录使用进程内锁、短期 POSIX 目录锁、跨网络期 BSD 记录租约、不覆盖原子发布以及文件/目录 `fsync`；macOS 门禁已覆盖同进程多实例、子进程继承锁与崩溃释放，但尚未在 iPad 上做拔电式硬断电验证，也没有 App Group 容器，因此不能宣称跨扩展共享或绝对掉电耐久。记录会保守累积到 1,000 份上限，当前没有自动清理或用户解锁界面。
- v2 持久化屏障已经阻止同一 broker 账号作用域在同一容器内的重启盲重试，并允许不同作用域各自保留记录；账户身份探针解析的 UK 明确不会保存为或转换成该作用域。只读列表观察可以报告“单个短页本次未观察到但缺席未证明”“元数据冲突”“元数据一致但内容未证明”或“不确定”；完整字节核对核心虽已能在隔离测试中流式比较 SHA-256，但仍没有真实 broker、生产调用入口、真实账号/CDN 联调或绑定 repository lease 的核销协议，因此不能删除记录、证明历史上传不存在或授权盲重试。当前没有后台传输能力，进入后台只允许协作式取消，不能承诺继续上传。
