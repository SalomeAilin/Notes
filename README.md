...
>
> 历史说明：初始开发包曾使用“墨记”，该名称已从后续可构建产品的显示名中移除；“墨计”没有作为当前产品展示名，它属于同音易混淆别名，发布门禁快照还记录了公开市场同名应用，因此与繁体写法一并列入禁用门禁。若设备仍显示任一退休名称，应停止该产物的验收并核对签名包的精确 Git commit 与 `provenance.json`，不能据此反推当前源码名称。上述工程门禁不等于商标清查或法律意见。
