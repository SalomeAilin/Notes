...
- 普通笔记继续生成历史兼容备份；内容超过旧格式容量时自动改用新版，文件名和操作不变，新版应用仍可导入旧备份。
...
- 备份文件扩展名始终为 `.notesbackup`。普通内容继续生成 v1（整包 32 MiB、单页 8 MiB）；只有 v1 装不下时才自动生成 v2。v2 把每页原稿按规范 1 MiB 块连续编排，逐块和逐页双重校验，当前过渡上限为整包 128 MiB、单页 64 MiB。导入端同时接受 v1/v2，恢复仍追加副本且不覆盖现有笔记。
...
- 连续长纸已启用按需分段索引、原稿分块、原子发布、双代有界回收、长页惰性迁移和可见范围读取的本地路径；短页不会承担分段成本。兼容备份已能在 v1 容量不足时自动使用 v2，并保持 v1 导入。当前画布和备份导出仍会一次载入完整页面/归档，单页与整包仍有过渡上限；区域画布、流式备份导出和分区恢复完成前，不宣称数据容量真正无限。
