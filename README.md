...
- UInfo 账户身份探针仍不负责签发账号作用域，也未接入 App、View 或 Store。当前核心新增了隔离的同账号只读观察器：它只按固定父目录、最多 10 页/1,000 项调用官方 `list`，以 UTF-8 字节精确匹配绝对路径，并把重复条目、`fs_id` 漂移、分页上限、畸形响应和账号错配全部失败关闭。普通 offset 分页不是服务端一致快照，因此多页未命中只能返回“不确定”；单个短页未命中也明确标为“缺席未证明”。即使路径、大小和云端哈希都匹配，也只返回“元数据一致但内容未证明”，不会删除 WAL、授权重传或标记成功；官方列表的云端哈希不能替代本地归档字节的 SHA-256/MD5 证明。仓库仍没有真实 broker、生产凭据发行器、自动核销、TTL 或“强制清除”入口。
- 当前核心另有未接 UI 的完整字节核对器：它按同账号记录与 `fs_id` 查询官方 `filemetas`，只把短期 access token 发送给固定的 `pan.baidu.com` 元数据端点和首跳 `d.pcs.baidu.com` 下载主机；后续只允许不再携带 token 的 HTTPS 重定向，并移除认证、Cookie、Referer、Origin 与 Range。下载使用无持久缓存的临时会话，拒绝压缩和分段响应，在 32 MiB 上限内流式计算完整 SHA-256；只有账号、备份、尝试、路径、官方 `filename`、大小、字节数和 SHA-256 全部一致才返回内存中的 `contentVerified`。该核对器没有 repository、lease、`FileManager` 或删除能力，不能核销 WAL；当前应用产物也没有生产凭据发行器或调用入口，尚未用真实百度账号验证 dlink/CDN 行为。
